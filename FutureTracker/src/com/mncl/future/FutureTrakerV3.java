...
public class FutureTrakerV3 {
	private  ArrayList<OutPutBean> outputList;
	private String outputFileName;
	public FutureTrakerV3(String outputFileName){
	...
	private void startTracking(InputBeen inputObj) throws IOException {
		...
		for(File csvFiles:files){
			...
			{				
				...
				if(isFirstDay){					                   
					...
					if(currentTime.equals(buyTime)||
						...
						/*if(exitPriceMap.containsKey(rowData[0].trim())){
							System.out.println(exitPriceMap.get(rowData[0].trim()).toString());
							count++;
						}*/
			...
			/*for (Entry<String, FutureBeen> entry : exitPriceMap.entrySet())
			{
	...
	public static void main(String[] args) {
		...
		InputBeen inputObj=new InputBeen(starttime, buyTime, pos_Square_Off_StartTime,
				Pos_Square_Off_EndTime, endTime, noOfTopToPick, inputFutureDataDir);
		FutureTrakerV3 futureTracker=new FutureTrakerV3(outputFileName);
		try {
